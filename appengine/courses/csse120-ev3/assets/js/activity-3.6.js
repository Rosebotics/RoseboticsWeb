var activity = [
  ...
  "<br><br><b>3.</b> Fill in the blank: Under the hood MQTT messages have a topic and a message that are both strings. We format the topic like a hierarchical folder path and we format the message as JSON, so that we can send more complex messages.  Within that JSON message there are two fields.  The first field is called '______', which is a string that must match the name of a method to call on the other end.<br>",
  {
